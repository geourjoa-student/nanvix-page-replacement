 ...
 */
#define getpte(p, a) (&((struct pte *)((getpde(p, a)->frame << PAGE_SHIFT) + KBASE_VIRT))[PG(a)])
...
PRIVATE void ageUp(){
	int i;
	struct pte *current_pte; /* Needed to access to accessed */
	for (i = 0; i < NR_FRAMES; i++){
		...
		if(current_pte->accessed==1)
		{
			...
			frames[i].not_used_for=0;
		}
		else {
			frames[i].not_used_for++;
		}
	}
}
...
PUBLIC void mappgtab(struct process *proc, addr_t addr, void *pgtab)
{
	...
	//KBASE  Base de l'offset de la memoire disponi 0 OS KBASE USER 0xXC0000000 OS END 
